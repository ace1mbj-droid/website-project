function DynamicFontScaler({
  ...
  function fits(scaler) {
    return (
      getContentWidth(scaler) <= container.clientWidth &&
      getNumLines(scaler) <= maxLines
    );
  }
  ...
  function getScalers() {
    return Array.prototype.slice
      .call(
        container.querySelectorAll(`[data-scaler-id="scaler-${containerId}"]`)
      )
      .sort(
        (first, second) =>
  ...
  function getContentWidth(el) {
    const paddingLeft = parseInt(
      getComputedPropertyValue(el, "padding-left") || 0,
      10
    );
    const paddingRight = parseInt(
      getComputedPropertyValue(el, "padding-right") || 0,
      10
    );
  ...
  function getNumLines(el) {
    ...
    const lineHeight =
      parseInt(getComputedPropertyValue(el, "line-height"), 10) || 1;
