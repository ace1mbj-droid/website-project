import { get, merge, reduce, startsWith } from "lodash";
import { getRichTextCharCount } from ".";
...
export function reduceFontSizeMap({
  ...
  return reduce(
    ...
    (result, value, key) => {
      if (startsWith(key, "@")) {
        result[key] = reduceFontSizeMap({ fontSizeMap: value, children });
      } else {
        let count;
...
export function resolveFontSizeMap(props) {
  return { style: reduceFontSizeMap(props) };
}
...
export function mergeTypographyOverrides(value, props) {
  return merge(
    ...
    resolveValue.call(
      this,
      this.mappedValues.typographyOverrides?.[value],
      props
    ) || {},
...
export function resolveStateLookup(state, props, defaultStyle = {}) {
  if (
    !state ||
    (props.typography && props.typography.toLowerCase()) ===
      (state && state.toLowerCase())
  )
    return defaultStyle;
  return (
    mergeTypographyOverrides.call(this, props.typography, props)[
      state.toLowerCase()
    ] || defaultStyle
  );
}
    ...
    .reduce(
      (finalState, curState) =>
        merge(
          {},
          finalState,
          mergeTypographyOverrides.call(this, curState, props)
        ),
      {}
    );
}
