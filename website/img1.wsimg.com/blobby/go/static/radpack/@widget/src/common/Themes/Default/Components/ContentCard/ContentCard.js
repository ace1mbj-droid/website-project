...
export function ContentCard({
  ...
  columnCountMap,
  isEqualHeightSm = false,
  isEqualHeightMd = false,
  ...props
}) {
  ...
  const headingHeightSyncStyles = {
    width: "inherit",
    top: 0,
    ...(isEqualHeightMd && { "@md": { position: "absolute" } }),
    ...(isEqualHeightSm && { "@sm-only": { position: "absolute" } }),
  };
...
function evalHeightPerLine(cards = [], numColumns) {
  const chunkGroups = (cards, perLine) => {
    const rows = [];
    for (let i = 0; i < cards.length; i += perLine) {
      rows.push(cards.slice(i, i + perLine));
    }

    return rows.map((row) => {
      const ratios = row.map(
        (item) =>
          Number(item?.image?.imageData?.outputWidth ?? 0) /
          Number(item?.image?.imageData?.editedAspectRatio || 1)
      );
      return new Set(ratios).size === 1;
    });
  };

  return {
    sm: chunkGroups(cards, 2),
    md: chunkGroups(cards, numColumns),
  };
}
...
export function ContentCards({
  ...
  const heightsPerLine = evalHeightPerLine(cards, cardsPerLine);
  ...
  const data = cards.map((child, i) => {
    ...
    const isEqualHeightMd = heightsPerLine.md[getChunkGroup(cardsPerLine, i)];
    const isEqualHeightSm = heightsPerLine.sm[getChunkGroup(2, i)];

    const longestHeadingMap = new Map();
    if (cards.length > 1) {
      if (mdTitle && isEqualHeightMd) {
    ...
    return (
      <UX2.Component.ContentCard
        ...
        columnCountMap={columnCountMap}
        isEqualHeightMd={isEqualHeightMd}
        isEqualHeightSm={isEqualHeightSm}
        {...otherProps}
      />
    );
  });
