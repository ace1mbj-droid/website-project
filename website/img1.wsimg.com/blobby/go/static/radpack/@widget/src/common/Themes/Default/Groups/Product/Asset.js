...
import { replaceImageSizeParams } from "../../../../utils/image";
...
export function ProductAsset({
  ...
  const oldAssetUrl =
    assets && assets.length ? assets[0].large_url : noProductImage;
  ...
  const imageUrl = replaceImageSizeParams(
    assetUrl,
    "/:/rs=w:{width},h:{height},cg:false,m"
  );
        ...
        this.merge(
          {
            ...
            children: [
              <UX2.Component.Background
                ...
                style={styles.inner}
                data-aid={dataAids.image}
              />,
              banner && banner.text !== null && (
                <UX2.Group.Product.Banner
                  key="banner"
                  getStyles={getStyles}
                  data-aid={dataAids.saleBanner}
                >
                  {banner.text}
                </UX2.Group.Product.Banner>
              ),
        ...
        this.merge(
          {
            group: "Product",
            groupType: "Asset",
            src: replaceImageSizeParams(
              assetUrl,
              `/:/rs=w:${unboxedSize},h:${unboxedSize}`
            ),
    ...
    this.merge(
      {
        style: {
          ...
          width: "auto",
          paddingVertical:
            getStyles({ saleBannerPaddingVertical: true }) || "xsmall",
