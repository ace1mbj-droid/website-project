...
export function CommerceCardPicture({
  ...
  const backgroundProps = {
    ...
    imageData: { overlayAlpha: overlayAlpha },
    style: {
      ...baseStyles,
  ...
  const imageProps = {
    ...
    style: {
      ...baseStyles,
      objectFit: ["shrink_to_fit", "shrinkToFit"].includes(imageCropMethod)
        ? "contain"
        : "cover",
    },
  };

  const PictureComponent = backgroundAssetUrl
    ? UX2.Component.Background
    : UX2.Element.Image;
  const pictureProps = backgroundAssetUrl ? backgroundProps : imageProps;

  const content = (
    <PictureComponent {...pictureProps} data-aid={dataAids.image} />
  );
