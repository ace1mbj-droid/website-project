...
import { ICON_SIZE, NAV_DRAWER_OFFSET } from "../../../../../constants";
...
class Hamburger extends Component {
    ...
    if (open !== this.state.open) {
      if (typeof window !== "undefined") {
        window.dispatchEvent(
          new Event(`NavigationDrawer${this.state.open ? "Opened" : "Closed"}`)
        );
      }
  ...
  setNavigationOffset = () => {
    ...
    this.toggleTarget.style.cssText = `
          ...
          transform: ${
            open
              ? `translateX(-${this.offsetLeft}px)`
              : `translateX(${NAV_DRAWER_OFFSET})`
          };
        `;
  };
  ...
  renderIcon() {
    const { icon, openIcon } = this.props;
    const { open } = this.state;

    if (open && openIcon === "close")
      return <UX2.Element.CloseIcon size={ICON_SIZE} />;
    return (
      <UX2.Element.Icon.Hamburger
  ...
  render() {
    ...
    const { style, staticContent = {} } = props;
    return (
      <Toggle
        {...props}
        ...
        style={{ border: 0, justifyContent: "flex-start", ...style }}
        data-aid={dataAids.HAMBURGER_MENU_LINK}
        aria-label={
