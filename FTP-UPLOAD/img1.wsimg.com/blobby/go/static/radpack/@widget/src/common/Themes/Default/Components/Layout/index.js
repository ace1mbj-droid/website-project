...
export function SplitLayout({
  ...
  const gridMarginOffset = this.parseSizedUnit(
    this.mapPropValue("spacing", "medium")
  )[0];
  ...
  const slot2 = mediaElement ? (
    <UX2.Component.Grid.Cell
      key="slot2"
      style={{ flex: 1, "@md": { flex: 1 } }}
    >
      <UX2.Element.Block
        style={{ ...fluidStyles, ...bleedStyles, position: "relative" }}
      >
        {mediaElement}
      </UX2.Element.Block>
    </UX2.Component.Grid.Cell>
  ...
  return this.merge(
    {
      ...
      fluid,
      children: (
        <UX2.Component.Grid xs={1} md={slots.length || 1} {...gridProps}>
...
export function VerticalBox({ align, ...props }) {
...
export function HorizontalBox({ align, ...props }) {
  return this.Box(
    this.merge(
      {
        alignX: align || "left",
...
export function Box({ alignX, alignY, ...props }) {
  const alignment = {
    top: "flex-start",
    left: "flex-start",
    start: "flex-start",
